...
const checkInvalidToken = (data) => {
	if(data.code ==='E100000' && data.msg === 'Error Token') {
		const cookieParam = {expires: -1, path: '/', domain: config.COOKIE_SOUPAI_DOMAIN}
		cookie.remove(config.COOKIE_SOUPAI_TOKEN, cookieParam)
		cookie.remove(config.COOKIE_SOUPAI_USER_NAME, cookieParam)
		cookie.remove(config.COOKIE_SOUPAI_USER, cookieParam)
