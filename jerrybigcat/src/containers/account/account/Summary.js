...
import { Loop, Button, Switch, Icon, Colors, Sizes } from '../../../components/m1ui'
...
class Summary extends Component {
	...
	loadSummary = () => {
		...
		getSummary().then(result => {
			...
			if(result.success) {
				...
				const money_balance = data.balance
				const edition = account.edition
				const expireTime = account.gmtExpire
				const startTime = account.gmtStart
				...
				const isOpenLogoEdition = [2,3].indexOf(edition) >= 0
				// const hideM1Logo =  isOpenLogoEdition ? true : (data.mservice.length && data.mservice[0] === 1)
				const hideM1Logo = false
	...
	handleHideM1LogoChange = (e) => {
		...
		if(user.cookieInfo.user.accounts[0].edition === 1) {
		...
		if(this.state.isOpenLogoEdition) {
			msg.info('您当前使用版本已自动开启隐藏M1商标服务')
			return
		}

		if(user.cookieInfo.user.accounts[0].edition === 4) {
	...
	renderExpire() {
		const { useDayPercentage, validDay, formatExpireTime, edition } = this.state

		return (
			<div className="info-box box-large">
				<h2><span className={classNames('icon', user.filter.versionKeys[edition])}><Icon name={user.filter.versionIconNames[edition]}/></span>M1云端市场部</h2>
				<div className="m1-row">
					<div className="summary-intro-content">
						<Loop r={96} value={useDayPercentage} width={6} showValue={true} text="已使用" />
				...
				{user.cookieInfo.user.accounts[0].edition === 1 &&
