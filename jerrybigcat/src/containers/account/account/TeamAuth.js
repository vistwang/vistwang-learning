...
import { Modal, Button, ButtonGroup, Checkbox, Radio, Tag, Text, Icon, Colors, Sizes } from '../../../components/m1ui'
...
class TeamAuth extends Component {
	...
	handleAccountTypeChange = accountType => {
		...
		this.submitToggleHighAuth((accountType === AccountTypes.GENERAL) && this.state.showHighAuth)
	}
	...
	handleRequestSearch = app => {
		user.getAuthSuggestion(app, data => {
			...
			if(this.contact_gid > 0) {
				...
				this.setState({
					showHighAuth: true,
					accountType: AccountTypes.GENERAL
				})
	...
	renderSingleAuth(app) {
		...
		const emptyContent = <div className="empty-content">
													<img className="no-add" src={emptyImage}/>
    ...
    return (
      <div className="m1-row">
          <div className="team-set-auth">
              ...
              <p>
                  <Radio 
                      checked={accountType === AccountTypes.GENERAL} 
                      onChange={e => this.handleAccountTypeChange(AccountTypes.GENERAL)} 
                      text={'邀请账号可拥有部分权限'} />
              </p>
              </div>
              {accountType === AccountTypes.GENERAL &&
