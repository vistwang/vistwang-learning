...
import { Button, Text, Alert, Tag, Icon, Input, Colors, Sizes } from '../../../components/m1ui'
...
class Auth extends Component {
	...
	setBaseInfo(info) {
		const data = info.base || info.base_formal
		if(data) {
			this.setState({
	...
	setSmsInfo(info) {
		const data = info.sms || info.sms_formal
		if(data) {
			this.setState({
	...
	handleSMSAuthClick = e => {
		if(!this.certifyInfo.base_formal) {
	...
	render() {
		const baseInfo = this.certifyInfo.base || this.certifyInfo.base_formal || {}
		const smsInfo =  this.certifyInfo.sms || this.certifyInfo.sms_formal || {}
		return (
					...
					<div className="m1-row auth-alert">
						<Alert color={Colors.PRIMARY}>
							...
							<ul className="m1-ul">
								...
								<li>如您需要使用短信营销服务，请仔细阅读M1云端市场部<a href="http://www.m1world.com/terms.html#terms_sms" target="_blank">短信发送服务协议</a></li>
							</ul>
						</Alert>
					</div>
				</div>
