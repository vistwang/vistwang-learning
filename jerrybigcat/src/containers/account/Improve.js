...
import { Input, Button, Icon, Colors, Sizes } from '../../components/m1ui'
...
class Improve extends Component {
	...
	init() {
		cookie.remove(config.COOKIE_SOUPAI_FIRST_LOGIN,{path: '/', domain: config.COOKIE_SOUPAI_DOMAIN})
	...
	handleSaveClick = e => {
		...
		if(username.trim().length === 0) {
			...
			if(this.industryKey) {
				paramOption.industryKey = this.industryKey
			}
