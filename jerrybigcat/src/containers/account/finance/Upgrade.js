...
class Renew extends Component {
	...
	initVersionState(v) {
		...
		if(!v) {
			// const currEdition = user.cookieInfo.user.accounts[0].edition
			const currEdition = 1
			v = qs.get('v') || (currEdition === 1 ? 2 : currEdition)	// 	默认当前版本, 免费版默认为黄金版
	...
	getBalance = () => {
		...
		getBalance().then(result => {
			...
			if(result.success) {
				this.computedMoney({
					...this.state,
					balance: result.data.moneyBalance,
	 ...
	 */
	alipay() {
		const { edition, month, payGold, totalPayMoney, otherPayMoney } = this.state
		let paramUrl = `&v=${edition}&u=${month}` + (payGold > 0 ? `&gold=${payGold}` : '')
		const param = {
			totalFee: totalPayMoney,
			callbackUrl: config.DOMAIN + '/account/price/alipay.html?',
			scope: paramUrl,
			for: PayFor.UPGRADE
		}
		alipay(param)
	 ...
	 */
	wxpay() {
		const { edition, month, otherPayMoney, payGold } = this.state
		const paramUrl = `&v=${edition}&u=${month}` + (payGold > 0 ? `&gold=${payGold}` : '')
		msg.loading()
		wxpay({
	...
	upgradeVersion = () => {
		const {edition, month } = this.state
		msg.loading()
		user.upgradeVersion({edition, unit: month}, result => {
			msg.close()
			if(result.success) {
				msg.info('升级成功')
				setTimeout(() => {
	...
	render() {
		const { edition, month, trade_id, upgradeType, totalPayMoney, gift, otherPayMoney, balancePayMoney, goldPayMoney, goldMoney, payGold, payWay, balance, gold, coupon, 
			...
			<div className="m1-panel">
				...
				<div className="m1-panel-content">
					<div className="recharge-content">
						<UpgradeVersion version={edition} type={upgradeType} onSelect={this.handleUpgradeVersionSelect} />
