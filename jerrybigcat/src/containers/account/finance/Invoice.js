...
import { Button, Tag, Text, Colors, Sizes } from '../../../components/m1ui/'
	...
	applyInvoice = list => {
		...
		list.forEach(item => {
			let invoice = {
				...
				phone: setting.phone,
				taxpayerIdentity: setting.taxpayerIdentity
			}
			if(setting.type === 2) {
				invoice.registeredPhone = setting.registeredPhone
				invoice.registeredAddress = setting.registeredAddress
				invoice.bank = setting.bank
				invoice.bankAccount = setting.bankAccount
			}
