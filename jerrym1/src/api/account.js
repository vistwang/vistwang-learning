...
export function updateAccountName(name) {
	const url = m1api + '/account/update'
	const param = {
		token,
		account_name: name
	}
...
export function sendTelCode(tel) {
	const url = m1api + '/bind/send_code/ex'
	const param = {
		token,
		mobile: tel
	}
 ...
 * @param  {String} code 
 */
export function verifyTelCode(tel, code) {
	const url = m1api + '/bind/verify'
	const param = {
		token,
		mobile: tel,
		verify_code: code
...
export function sendBindEmail(email) {
	const url = m1api + '/user/bind_email'
	const param = {
		token,
		email
	}
...
export function modifyPassword(oldPwd, newPwd) {
	const url = m1api + '/user/set_password'
	const param = {
		token,
		oldpassword: oldPwd,
		password: newPwd
 ...
 * @param  {String} options.fileStr basee64 选择file 此参数可不传
 * @param  {String} options.pictype 目前两种tx 头像，wx 微信二维码
 * @return {[type]}                 [description]
 */
export function uploadFile({file, fileStr, pictype}) {
	const url = m1api + `/user/upload/${pictype}`
 ...
 * @param  {String} for          类型 充值charge 升级upgrade 消费consume
 ...
 * @param  {String} options.type      充值 charge 升级 upgrade
 * @param  {[type]} options.scope     [description]
 * @return {[type]}                   [description]
 */
export function bankpay({total_fee, type, scope}) {
	const url = m1api + '/bankpay/pay'
	const param = {token, total_fee, for: type, scope}
	return http.post(url, param)
}
/**
 * 金币抵扣
 ...
 */
export function upgrade({version, unit, code}) {
	const url = m1api + '/upgrade'
	const param = {token, version, unit, code}
	return http.post(url, param)
}
...
export function getApplyInvoiceList() {
	const url = m1api + '/trades_for_invoce_apply'
	const param = {token}
	return http.get(url, param)
}
/**
...
export function getInvoiceSetting() {
	const url = m1api + '/invoice/setting_detail'
	const param = {token}
	return http.get(url, param)
}
/**
...
export function saveInvoiceSetting(options) {
	const url = m1api + '/invoice/save_setting'
	const param = {token, ...options}
	return http.post(url, param)
}
/**
 ...
 */
export function sendInvite(emails,type,detail) {
	const url = m1api + '/invite/send'
	const param = {token, emails, type, detail: JSON.stringify(detail)}
	return http.post(url, param)
}
/**
 * 重发邀请
 ...
 * @param  {String} detail 
 */
export function resendInvite(email, type, detail) {
	const url = m1api + '/invite/re_send'
	const param = {token, email, type, detail}
	return http.post(url, param)
}
/**
 * 编辑子账号、邀请权限
 ...
 */
export function editInvite({type,detail,invite_id,user_id}) {
	const url = m1api + '/invite/edit'
	const param = {token, type, detail: JSON.stringify(detail), invite_id, user_id}
	return http.post(url, param)
}
/**
 * 删除子账号、邀请
 ...
 */
export function removeInvite({user_id,invite_id}) {
	const url = m1api + '/invite/del'
	const param = {token, user_id, invite_id}
	return http.post(url, param)
}
/**
 * 搜索建议-获取所有权限
 ...
 */
export function improveInfo({realname, company, jobtitle, industry, industry_key}) {
	const url = m1api + '/user/perfect'
	const param = {token, realname, company, jobtitle, industry, industry_key}
	return http.post(url, param)
}
/**
 * 获取行业列表
...
export function getIndustry() {
	const url = m1api + '/industry'
	return http.get(url)
}
/**
 * 充值赠送活动
 */
export function getRechargeGift() {
	const url = m1api+ '/trades/gift'
	return http.get(url)
}