...
import { Button, Text, Checkbox, Modal, Colors, Sizes } from '../../../components/m1-ui/'
...
class InvoiceApply extends Component {
	...
	handleInvoiceApplyClick = e => {
		const invoices = this.state.applyInvoices.filter(item => {
			return !!item.checked
		})
	...
	computedTotalMoney = () => {
		...
		this.state.applyInvoices.forEach(item => {
			if(!!item.checked) {
				selectTotalMoney += item.total_fee
			}
		})
