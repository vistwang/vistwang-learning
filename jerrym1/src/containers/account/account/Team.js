...
import { Tag, Input, Radio, Checkbox, Button, Icon, Text, Colors, Sizes } from '../../../components/m1-ui'
...
class Team extends Component {
	...
	handleInviteResendClick = (id) => {
		const invite = this.state.invites.find(item => item.invite_id === id)
		msg.loading()
		resendInvite(invite.email, invite.type, invite.detail).then(result => {
			msg.close()
			if(result.success) {
				msg.info('邀请重发成功')
			} else {
	...
	handleAccountEditClick = (id, status) => {
		...
		const idKey = status === 1 ? 'userid' : 'invite_id'
		const postKey = status === 1 ? 'user_id' : 'invite_id'
	...
	renderAccountList(accounts, status) {
		...
		// 普通用户禁止编辑、重发、删除等操作
		const disabledOpera = accountType === AccountTypes.ORDINARY
		...
		const accountList = accounts.map((item, i) => {
			const id = status === 1 ? item.userid : item.invite_id
