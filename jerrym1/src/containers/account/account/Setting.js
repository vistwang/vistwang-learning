...
import { Input, Button, Icon, Text, Colors, Sizes } from '../../../components/m1-ui'
...
class Setting extends Component {
	...
	initSettingState = (data) => {
		...
		this.setState({
			...
			weiXinCode: userInfo.weiXinCode || '',
	...
	setSettingInfo(key, value) {
		if(value !== this.setting.userInfo[key]) {
			const userObj = {userId: this.setting.userInfo.userId, [key]: value}
			setSettingInfo(userObj).then(result => {
				if(result.success) {
					this.setting.userInfo[key] = value
					user.info[key] = value
				}
			})
		}
	...
	handleChangeFileQr = e => {
		...
		if(file) {
			...
			uploadFile({
				...
				if(result.success) {
					this.setState({
						weiXinCode: result.data.fileUrl
					})
				} else {
					msg.info(result.data)
				}
	...
	handleVerifyTelCodeToBind = () => {
		...
		if(!/^[0-9]{6}$/.test(telVerifyCode)) {
			...
			verifyTelCode(newMobilePhone, telVerifyCode).then(result => {
				...
				if(result.success) {
					msg.info('绑定手机成功')
					this.setting.userInfo.mobilePhone = newMobilePhone
					this.setState({
	...
	handleAvatarCropSubmit = imgUrl => {
		if(imgUrl) {
			...
			uploadFile({
				...
				msg.close()
				if(result.success) {
					this.setting.userInfo.avatar = result.data.fileUrl
					this.setState({
	...
	renderEmail() {
		...
		return(<div className="m1-form-row">
							<div className="field-info-wrap">
								...
								<div className="m1-form-content">
									<label className="m1-form-label-input" onClick={!showModifyEmail && this.handleClickModifyEmail}>{email ? email : <span>输入邮箱</span>}</label>
								</div>
		...
		return (
			<div className="m1-form-row">
				<div className="field-info-wrap">
					...
					<div className="m1-form-content">
						<label className="m1-form-label-input" onClick={!showModifyPhone && this.handleClickModifyPhone}>{mobilePhone ? mobilePhone : <span>输入手机</span>}</label>
					</div>
	...
	renderModifyPassword() {
		...
		return (<div>
			<div className="m1-form-row">
					...
					<span className="intro"><a href="">忘记密码？</a></span>
			</div>
			<div className="m1-form-row">
					<label className="m1-form-label"></label>
					<div className="m1-form-content">
