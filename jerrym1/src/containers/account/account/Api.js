...
import { Input, Button, Colors, Text, Tag, Alert } from '../../../components/m1-ui'
...
class Api extends Component {
	...
	setVersionInfo() {
		const version = user.cookieInfo.userinfo.accounts[0].version
		const versionName = user.filter.versionNames[version]
		const expireTimeStr = utils.formatDate(user.cookieInfo.userinfo.accounts[0].expireTime)
		this.setState({
			version,
			versionName,
			expireTimeStr
