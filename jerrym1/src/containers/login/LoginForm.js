...
import { Button, Icon, Checkbox, Colors, Sizes } from '../../components/m1-ui'
...
class LoginForm extends Component {
	constructor(props) {
		...
		this.sem = utils.getUrlParam('ref') ? (utils.getUrlParam('ref') === 'sem' ? 'm1-sem' : utils.getUrlParam('ref')) : 'm1'
	...
	init() {
		...
		if(cookie.get(config.COOKIE_M1_TOKEN)) {
			this.token = cookie.get(config.COOKIE_M1_TOKEN)
			this.bindCoupon()
		} else if(this.ref) {
	...
	isRememberPwd() {
		return localStorage.getItem(config.COOKIE_M1_AUTO_LOGIN) === 'true'
	}

	setRememberPwd(bool) {
		if(bool) {
			localStorage.setItem(config.COOKIE_M1_AUTO_LOGIN, 'true')
		} else {
			localStorage.removeItem(config.COOKIE_M1_AUTO_LOGIN)
		}
	}
	...
	loginResult = (result) => {
		...
		if(result.success) {
			...
			const domain = 'm1world.com'
			const token = result.data.token.toString()
			const userinfo = encodeURIComponent(JSON.stringify(m1.arrangeUserInfo(result.data)))
			const username = Base64.encode(result.data.userinfo.userName)
			const cookieOption = {expires, path: '/', domain}


			cookie.set(config.COOKIE_M1_TOKEN, token, cookieOption)
			cookie.set(config.COOKIE_M1_USER_NAME, username, cookieOption)
			cookie.set(config.COOKIE_M1_USER_INFO, userinfo, cookieOption)

			localStorage.setItem(config.COOKIE_M1_USER_NAME, username)
	...
	bindCoupon() {
		let coupon = cookie.get(config.COOKIE_M1_COUPON)
		if(!!coupon) {
		...
		if(result.success) {
			...
			if(!!coupon) {
				...
				if(coupon.name === 'coupon1') {
					...
					cookie.set(config.COOKIE_M1_COUPON, encodeURIComponent(JSON.stringify(coupon)), {expires: 365, path: '/', domain: 'm1world.com'})
				}
			}
		}
		redirect()
